local rawget = rawget
local rawset = rawset
local type = type
local getmetatable = getmetatable
local setmetatable = setmetatable
local require = require
local str_sub = string.sub

local GetUProperty = GetUProperty
local SetUProperty = SetUProperty
local RegisterClass = RegisterClass
local RegisterEnum = RegisterEnum
-- local print = UEPrint

_NotExist = _NotExist or {}
local NotExist = _NotExist

local function Index(t, k)
	local mt = getmetatable(t)
	local super = mt
	while super do
		local v = rawget(super, k)
        if v~= nil then
            if rawequal(v, NotExist) then
                return nil
            end
			rawset(t, k, v)
			return v
		end
		super = rawget(super, "Super")
	end
    local p = mt[k]

    if p ~= nil then
        if type(p) == "userdata" then
            return GetUProperty(t, p)
        elseif type(p) == "function" then
            rawset(t, k, p)
        elseif rawequal(p, NotExist) then
            return nil
        end
    else
        rawset(mt, k, NotExist)
    end
	return p
end

local function NewIndex(t, k, v)
	local mt = getmetatable(t)
	local p = mt[k]
	if type(p) == "userdata" then
		return SetUProperty(t, p, v)
	end
	rawset(t, k, v)
end

local function Class(super_name)
	print("call Class:", super_name)
	local super_class = nil
	if super_name ~= nil then
		super_class = require(super_name)
	end

	local new_class
	if super_class == nil then
		new_class = {}
		new_class.__index = Index
		new_class.__newindex = NewIndex
		new_class.Super = super_class
		new_class.ctor = function() end
	else
		new_class = clone(super_class)
		new_class.Super = super_class
	end

	function new_class.New(...)
		local instance = setmetatable({}, new_class)
		instance.class = new_class
		instance:ctor(...)
		return instance
	end

    return new_class
end

---@param super_name string
function _G.LuaUnrealClass(super_name)
	local super_class = nil
	if super_name ~= nil then
		super_class = require(super_name)
	end

	local new_class = {}
	new_class.__index = Index
	new_class.__newindex = NewIndex
	new_class.Super = super_class
	new_class.__className = "LuaUnrealClass";
	-- new_class.__this = new_class;

    return new_class
end

local function global_index(t, k)
	if type(k) == "string" then
		local s = str_sub(k, 1, 1)
		if s == "U" or s == "A" or s == "F" then
			RegisterClass(k)
		elseif s == "E" then
			RegisterEnum(k)
		end
	end
	return rawget(t, k)
end

if WITH_UE4_NAMESPACE then
	print("WITH_UE4_NAMESPACE==true");
else
	local global_mt = {}
	global_mt.__index = global_index
	setmetatable(_G, global_mt)
	UE4 = _G

	print("WITH_UE4_NAMESPACE==false");
end

--
--Create an class.
--
function _G.clone(object)
    local lookup_table = {}
    local function _copy(object)
        if type(object) ~= "table" then
            return object
        elseif lookup_table[object] then
            return lookup_table[object]
        end
        local new_table = {}
        lookup_table[object] = new_table
        for key, value in pairs(object) do
            new_table[_copy(key)] = _copy(value)
        end
        return setmetatable(new_table, getmetatable(object))
    end
    return _copy(object)
end

function _G.PrintTable ( Lua_table )  
    local function define_print(_tab,str)
        str = str .. "  "
        for k,v in pairs(_tab) do
            if type(v) == "table" then
                if not tonumber(k) then
                    print(str.. k .."{")
                else
                    print(str .."{")
                end
                define_print(v,str)
                print( str.."}")
            else
                print(str .. tostring(k) .. " = " .. tostring(v))
            end
        end
    end
    if type(Lua_table) == "table" then
        print("{")
        define_print(Lua_table," ")
        print("}")
    else
        print(tostring(Lua_table))
    end
end

function _G.TableSize ( Lua_table )
	if Lua_table and type(Lua_table) == "table" then
		local count = 0
		for i, v in pairs(Lua_table) do
			count = count + 1
		end
		return count
	end
	return 0
end

-- _G.print = print
-- _G.print = UEPrint
_G.Index = Index
_G.NewIndex = NewIndex
_G.Class = Class